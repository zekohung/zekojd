...
WxPusher_notify_api() {
    local appToken=$1
    local content=$(echo $2 | perl -pe '{s|(\")|'\\'\\1|g}')
    local summary=$3
    local uids=$4
...
content_notify(){
    ...
    log_one_to_one(){
        ...
        if [[ $(echo $WP_APP_TOKEN_ONE|grep -Eo 'AT_(\w{32})') ]]; then
            ...
            if [[ "$uid" ]]; then
                ...
                content="$content_0$ExNotify_Top_Content<br><br>$content_1$content_2$content_3$content_4$content_5<br><br><br><br>$ExNotify_Bot_Content"
    ...
    [[ $notify_content ]] && echo -e "# 推送通知..." && notify "Cookie 状态通知" "$ExNotify_Top_Content\n\n$notify_content\n\n$ExNotify_Bot_Content" >/dev/null 2>&1
    ...
    #    local content="Cookie 状态通知<br><br>$ExNotify_Top_Content<br><br>$(echo $display_content | perl -pe '{s|\\n|<br>|g}')<br><br><br><br>$ExNotify_Bot_Content"
